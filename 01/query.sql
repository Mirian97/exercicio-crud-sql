--1 - Sendo assim, faça a criação de um banco de dados em seu computador chamado `exercicio_crud_sql` para que seja possível executar as queries criada por vc e execute o script de criação da tabela `usuarios` abaixo:
